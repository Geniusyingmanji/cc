// openai-anthropic-proxy.js
// Accepts Anthropic Messages API requests and translates to standard OpenAI Chat Completions API.
// Usage: OPENAI_BASE_URL=... OPENAI_API_KEY=... OPENAI_MODEL=... PROXY_PORT=... node openai-anthropic-proxy.js

const http = require("http");
const https = require("https");
const { URL } = require("url");

const LISTEN_PORT = parseInt(process.env.PROXY_PORT || "18901");
const OPENAI_BASE_URL = process.env.OPENAI_BASE_URL; // e.g. http://35.220.164.252:3888/v1
const OPENAI_API_KEY = process.env.OPENAI_API_KEY;
const OPENAI_MODEL = process.env.OPENAI_MODEL || "hermes"; // actual model name to send to OpenAI endpoint
const MODEL_NAME = process.env.MODEL_NAME || "claude-sonnet-4-6"; // model name to report back to Claude Code
      ...
      if (data === "[DONE]") {
        ...
        if (toolIds.length > 0) {
          for (const id of toolIds) {
            send("content_block_stop", { type: "content_block_stop", index: toolCalls[id].index });
          }
...
function openaiRequest(oaiBody, onResponse) {
  const targetPath = baseUrl.pathname.replace(/\/$/, "") + "/chat/completions";
...
function handleMessages(req, res) {
  ...
  req.on("end", () => {
    ...
    console.log("[" + new Date().toISOString() + "] " + (isStream ? "stream" : "sync") + " request, model=" + OPENAI_MODEL + ", messages=" + oaiBody.messages.length + ", tools=" + (oaiBody.tools || []).length);

    openaiRequest(oaiBody, (apiRes, error) => {
      if (error) {
      ...
      if (apiRes.statusCode >= 400) {
        ...
        apiRes.on("end", () => {
          console.error("OpenAI error " + apiRes.statusCode + ":", errBody);
          res.writeHead(apiRes.statusCode >= 500 ? 500 : 400, { "Content-Type": "application/json" });
          res.end(JSON.stringify({
            type: "error",
            error: { type: apiRes.statusCode === 429 ? "rate_limit_error" : "api_error", message: errBody },
          }));
        });
        return;
      }
      ...
      if (isStream) {
        ...
        apiRes.on("end", () => {
          try {
            ...
            res.writeHead(502, { "Content-Type": "application/json" });
            res.end(JSON.stringify({ type: "error", error: { type: "api_error", message: "Failed to parse response" } }));
          }
        });
      }
    });
...
function handleModels(req, res) {
  ...
  if (model) {
    res.writeHead(200, { "Content-Type": "application/json" });
    res.end(JSON.stringify({ id: model, object: "model", created: Date.now(), owned_by: "openai" }));
  } else {
    res.writeHead(200, { "Content-Type": "application/json" });
    res.end(JSON.stringify({ data: [{ id: MODEL_NAME, object: "model", created: Date.now(), owned_by: "openai" }], object: "list" }));
  }
}
...
server.listen(LISTEN_PORT, "127.0.0.1", () => {
  console.log("Anthropic-to-OpenAI proxy listening on 127.0.0.1:" + LISTEN_PORT);
  console.log("Target: " + OPENAI_BASE_URL + " model=" + OPENAI_MODEL);
  console.log("Report as: " + MODEL_NAME);
});
